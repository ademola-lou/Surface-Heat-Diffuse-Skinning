...
// 	void diffuse_vertex_in_range(int thread_id, int thread_num)
// 	{
// 		// Calculate vertex heat in range
// 		for (int i = thread_id; i < (int)vertices.size(); i += thread_num) {
// 			for (int j = 0; j < (int)vertices[i].neibours.size(); j++) {
// 				int neibour_index = vertices[i].neibours[j];
				
// 				// Get heat from neibour vertex
// 				for (int k = 0; k < (int)bone_points.size(); k++) {
// 					float& heat = ping_pong ?
// 						vertices[i].bone_heat.ping_heats[k] :
// 						vertices[i].bone_heat.pong_heats[k];
					
// 					float neibour_heat = ping_pong ?
// 						vertices[neibour_index].bone_heat.pong_heats[k] :
// 						vertices[neibour_index].bone_heat.ping_heats[k];
					
// 					// Weight the heat by distance
// 					float distance = vertices[i].pos.distance(vertices[neibour_index].pos);
// 					float weight = 1.0f / (1.0f + distance * max_fall_off);
// 					heat += neibour_heat * weight;
// 				}
// 			}
...
// 				heat /= (vertices[i].neibours.size() + 1) * 2.0f;  // Increase falloff
// 			}
// 		}
// 	}
...
// // Create a wrapper class for JavaScript interaction
...
//                 indices.call<void>("push", weight.bone);
//                 weights.call<void>("push", weight.weight);
//             }
//         }
